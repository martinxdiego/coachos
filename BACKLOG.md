...
- [ ] **S5.2 (P2, 8 SP) Alle UI-Strings extrahieren** 🔄 in Arbeit (flächenweise)
  ...
  *Fortschritt:* Muster etabliert (Server: `getTranslations`, Client: `useTranslations`, lenienter Fallback). **Öffentlicher Flow vollständig DE+EN:** `auth`=Login (`ac863ff`), `join`=/join (`6a2f388`), `register`=Selbstregistrierungs-Formular (`b4a10ef`). **Coach-Header alle 13 DE+EN** (`pages`-Namespace: `8ebbd75`+`c6a73de`). **Offen (flächenweise):** tiefere Strings je Coach-Seite (Buttons, Empty-States, Formularlabels — der Großteil), `/p` Spieler-Bereich, Action-Fehler, Push-Texte, PDF-Labels. Jede Fläche einzeln verifiziert.
