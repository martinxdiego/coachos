...
- [ ] **S5.2 (P2, 8 SP) Alle UI-Strings extrahieren** 🔄 in Arbeit (flächenweise)
  ...
  *Fortschritt:* Muster etabliert (Server: `getTranslations`, Client: `useTranslations`, lenienter Fallback). **Öffentlicher Flow vollständig DE+EN:** `auth`=Login (`ac863ff`), `join`=/join (`6a2f388`), `register`=Selbstregistrierungs-Formular (`b4a10ef`). **Coach-Header alle 13 DE+EN** (`pages`: `8ebbd75`+`c6a73de`). **Spieler-Startseite `/p` DE+EN** (`player`-Namespace inkl. ICU-Plural/Interpolation, `5882c72`). **Offen (flächenweise):** /p-Sub-Komponenten (Check-in-Card, Coach-Inbox, Saisonblatt, Notiz-an-Trainer, Install-Prompt), tiefere Strings je Coach-Seite (Buttons, Empty-States, Formularlabels — der Großteil), Action-Fehler, Push, PDF. Jede Fläche einzeln verifiziert.
